...
(function () {
  ...
  function pageActuals() {
    return '<h1 class="pg">Actual Values</h1>' +
      '<p class="lead dim">The monitored numbers are the machine telling you the truth. Values shown are from your own screen photos.</p>' +
      '<div style="height:14px"></div><div class="stack">' +
      DB.actuals.map(function (a) {
        return listRow("#/actual/" + a.id, a.name, null, vchip("yours", a.yourScreen));
      }).join("") + "</div>" +
  ...
  function pageActual(id) {
    ...
    return '<span class="eyebrow">Actual value</span><h1 class="pg">' + esc(a.name) + "</h1>" +
      '<div class="chips">' + vchip("your screen", a.yourScreen) + "</div>" +
